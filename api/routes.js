...
router.post('/users', asyncHandler(async (req, res) => {
  try {
    ...
    res.location('/');
    res.status(201).end(); // End without content
  } catch (error) {
...
router.put('/courses/:id', authenticateUser, asyncHandler(async (req, res) => {
  const course = await Course.findByPk(req.params.id);
  if (course) {
    console.log('Course Data:', course); // Logs the entire course object
    console.log('Course Owner ID:', course.userId); // Logs the owner ID of the course
    console.log('Authenticated User ID:', req.currentUser.id); // Logs the ID of the  authenticated user
